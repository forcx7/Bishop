...
def get_step(id_info, information):
    ...
    states = np.zeros([num_av, 2+10*3])
    adjacency = np.zeros([num_agents, num_agents])
    mask = np.zeros(num_av)
    ...
    if AVs: # 如果出现AV
        ...
        for av_id in av_ids:
            ...
            for other_id in ids:
                ...
                if distance < 20:
                    rel_pos = [pos[0] - veh[0], pos[1] - veh[1]]
                    rel_speed = traci.vehicle.getSpeed(other_id) - speed
                    nearby.append(rel_pos + [rel_speed])
            # Limit to a maximum number of nearby vehicles, pad if necessary
            max_nearby = 10  # Example
            while len(nearby) < max_nearby:
                nearby.append([0, 0, 0])
            nearby = nearby[:max_nearby]  # 截断列表，确保只有前十量
            observation = np.array([speed, lane] + [item for sublist in nearby for item in sublist], dtype=np.float32)
            # 合并自身信息&周边车辆信息
        ...
        # 生成邻接矩阵
        xs_av = np.array(Pos_av_x).reshape(-1, 1)  # 转换为numpy形式的列向量
        ys_av = np.array(Pos_av_y).reshape(-1, 1)
        dist_matrix_x = euclidean_distances(xs_av)
        dist_matrix_y = euclidean_distances(ys_av)
        ...
        adjacency[:len(AVs), :len(AVs)] = adjacency_small[:len(AVs), :len(AVs)] # 不论有无AV，格式必须固定，需要倒腾矩阵
        ...
        for i in range(num_av):
            agent_id = f"t_{i}"
            if agent_id in av_ids:  # 检查该智能体是否存在
                mask[i] = 1  # 如果存在，设置为1
            else:
                mask[i] = 0  # 如果不存在，设置为0
...
def check_state(rl_actions):
    ...
    for ID in ids:
        ...
        rl_actions2[rl_actions2 >2] = 1 # 原理明白，但合适吗？？？

        # print(next_lane)
        rl_actions3[rl_actions3< 3] = 14 # 避免加减速幅度过大
        ...
        # rl_actions3 = rl_actions3[num_hv:num_hv + len(AVs)] #只关心AV
        if len(AVs) != 0:
            next_lane = np.clip(current_lane + rl_actions2[NO] - 1, 0, 2)
            # print("Length of rl_actions2:", len(rl_actions2))
    ...
    id_list = {'ids': ids, 'EdgeList': EdgeList, 'exist_V': exist_V, 'AVs': AVs, 'HVs': HVs, 'Pos_x': Pos_x, 'Pos_av_x': Pos_av_x,
               'Pos_y': Pos_y, 'Pos_av_y': Pos_av_y, 'Vel': Vel, 'LaneIndex': LaneIndex, 'Edge': Edge, 'drastic_veh': drastic_veh,
...
if Training:
    ...
    #创建多智能体各自的网络结构
    GRL_Nets, agent = Create_DQN(num_hv, num_av, info_dict) # GRL_Nets就没有被使用过，在这里
    ...
    for i in range(1, n_episodes + 1):
        ...
        if __name__ == "__main__":
            ...
            traci.start([sumoBinary , "-c", "Env_Net/roundabout.sumocfg"])
            #初始化actions动作
            num_agents = num_av + num_hv
            actions = np.zeros(num_av)  # 生成original动作
            list_info = check_state(actions) # 执行动作（变道换速），返回环境信息
            obs = get_step(list_info, info_dict) # 返回节点特征矩阵、邻接矩阵&mask矩阵
            R = 0  # 行为奖励
            t = 0  # 时间步长
            A_e=0
            A_s1=[]
            ...
            for step in range(0, 4000):
                ...
                if warmup_count <= Warmup_Steps:  # 进行warmup
                    actions = np.random.choice(np.arange(num_actions), num_av)  # 生成随机动作
                else:
                    for i in range(num_av): # agent与环境进行交互
                        # if i < num_hv:
                        #     actions[i] = 0
                        # else:
                        action = agent.choose_action(obs, i) # 每个AV都有Q网络，但是输入的Node包含了其他AV的信息，错误的！！！
                        actions[i] = action[i] # 为什么是action[i]呢？？？
                ...
                for id in ids:
                    if traci.vehicle.getTypeID(id) == 'AV':
                        AV_energy += traci.vehicle.getElectricityConsumption(id) # 电能消耗，好像没用到？？？
                        exist_AV.append(id)
                ...
                warmup_count += 1
                done = (len(exist_AV) == 0) and(step>1000) # 刚过1000没多久，AV就走光了
                # ------将交互结果储存到经验回放池中------ #
