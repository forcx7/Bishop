...
class DQN(object):
    ...
    def store_transition(self, state, action, reward, next_state, done, i):
        """
           <经验存储函数>
           用来存储agent学习过程中的经验数据
    ...
    def sample_memory(self, i):
        """
           <经验采样函数>
           用来从agent学习过程中的经验数据中进行采样
        """
        # 调用replay_buffer中的采样函数
        data_sample = self.replay_buffers[i].sample(self.batch_size, self.n_steps)
        return data_sample
    ...
    def choose_action(self, observation, i):
        ...
        # action = self.models[i - num_hv](observation)
        action = self.models[i](observation)
        # print("action_pre:",action)
        action = torch.argmax(action, dim=1)      # 取每一行（每个agent）最大q值对应的动作(greedy_action)
        # print("action_aft:",action)
        action = self.explorer.generate_action(action)
        # print("action_end:",action)
        return action
    ...
    def compute_loss(self, data_batch, i):
        """
           <损失计算函数>
        ...
        for elem in data_batch:
            ...
            # 计算当前网络预测值
            q_predict = self.models[i](state)  # 计算状态S的Q表
            # q_predict = q_predict.gather(1, action.unsqueeze(1)).squeeze(1)  # 根据action（AV的动作）选择Q预测值（某个AV的AV+HV的Q值）
            q_predict = q_predict[0, action] # 取出第一行（AV）对应索引的值

            # 储存q_predict
            q_predict_save = q_predict.detach().cpu().numpy().reshape(-1)
            data_useful = np.any(q_predict_save)
            self.q_record.append(q_predict_save / (data_useful.sum() + 1))

            # 计算目标网络目标值
            q_next = self.target_models[i](next_state)
            q_next = q_next[0].max(dim=0)[0] # 提取第一行（AV）的最大值及索引
    ...
    def soft_update(self, i):
        """
           <目标网络soft更新函数>
           采用soft_update的方法同步目标网络（target_network）
        """
        # 软更新是指目标网络的参数通过加权平均的方式逐步向源网络的参数靠拢
        # 必须要定义正确的soft更新参数
        assert 0.0 < self.soft_update_tau < 1.0 # target net更新参数，控制目标网络更新的速度

        # 进行参数更新
        for target_param, source_param in zip(self.target_models[i].parameters(),
                                              self.models[i].parameters()):
            target_param.data.copy_((1 - self.soft_update_tau) * target_param.data +
                                    self.soft_update_tau * source_param.data)
    ...
    # def learn(self):
        ...
        for i in range(num_av):
            ...
            if f't_{i}' in exist_AV:
                ...
                samples, data_sample = self.sample_memory(i)

                # loss矩阵计算
                if self.n_steps == 1:  # 单步学习
                    elementwise_loss = self.compute_loss(data_sample ,i)
                else:  # 多步学习 还没有进行修改
                    elementwise_loss = self.compute_loss_multisteps(data_sample, self.n_steps)

                # 如果是Prioritized ReplayBuffer（优先buffer），则在计算总损失之前更新priority
                if isinstance(self.replay_buffers[i], PrioritizedReplayBuffer):
                    self.replay_buffers[i].update_priority(samples['indexes'], elementwise_loss)
    ...
    def save_model(self, save_path):
        ...
        torch.save(self.models, save_path)
    ...
    def load_model(self, load_path):
        ...
        self.models = torch.load(load_path)
