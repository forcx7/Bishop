    ...
    if f't_{i}' in av_ids:  # 如果出现AV
        ...
        for other_id in ids:
            ...
            if distance < 20 and nearby < max_nearby:
                ...
                if other_id in av_ids:
                    state.append([pos[0], pos[1], speed, lanes[0], lanes[1], 1])
                else:
        ...
        # 构造rmask矩阵，当av存在时值为1
        rmask = np.zeros(nearby + 1, dtype=int)
        for i in range(nearby + 1):
            if state[i,5] == 1:
                rmask[i] = int(1)
    return state, adjacency, rmask
...
def check_state(rl_actions, i):
    ...
    if f't_{i}' in AVs:  # 判断 't_i' 是否是 CAV
        ...
        if speed_change_action == 0:
            ...
            speed_change = 3  # 大幅加速
        # next_lane = np.clip(traci.vehicle.getLaneIndex(f't_{i}') + lane_change_action - 1, 0, 2)
        # traci.vehicle.changeLane(f't_{i}', next_lane, 100)  # 100代表变道时间是100ms
...
save_dir = 'GRL_TrainedModels/PPO' + now_time
try:
...
if Training:
    ...
    # 创建多智能体各自的网络结构
    agent = Create_PPO(num_hv, num_av, Graph)
    total_steps = 0
    ...
    for j in range(1, n_episodes + 1):
        ...
        if __name__ == "__main__":
            ...
            list_info = check_state(actions, 0)  # 执行动作（变道换速），返回环境信息
            ...
            for step in range(0, 4000):
                for i in range(num_av):  # agent与环境进行交互
                    ...
                    if warmup_count <= Warmup_Steps:  # 进行warmup
                        ...
                        if f't_{i}' not in exist_AV:
                            ...
                            action, prob, val = agent.choose_action(obs, i)  # PPO_agent，都只有AV的信息
                            if action.ndimension() == 0:  # 检查是否是标量张量
                                actions[i] = action.item()  # 转换为标量
                            else:
                                actions[i] = action[0]  # 多维张量时按原来的方式处理

                    list_info = check_state(actions[i], i)  # 执行动作（变道换速），返回环境信息
                    # obs_next = get_step(list_info, info_dict, i) # 在这里好像没用，返回下一时刻的obs
                    reward = get_reward(list_info, info_dict, i) # 返回奖励值
                    R += reward
                    done = (len(exist_AV) == 0) and (step > 500)
                    # ------将交互结果储存到经验回放池中，源代码逻辑如此------ #
                    if (warmup_count > Warmup_Steps) and (f't_{i}' in exist_AV):
                        agent.store_transition(obs, action, prob, val, reward, done, i) # PPO_agent
                    # ------环境观测更新，当前逻辑下obs是针对每个AV的，而非整体环境------ #
                    # obs = obs_next
                ...
                total_steps += 1
                # ------每100步Policy update------ #
                if (total_steps % agent.update_interval == 0) and (warmup_count > Warmup_Steps):
                    agent.learn(exist_AV)  # PPO_agent
            ...
            print("Loss:", Loss)
            if j % 1 == 0:
                print('Training Episode:', j, 'Reward:', R, 'Loss:', loss)
